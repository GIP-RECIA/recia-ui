...
export class ReciaTablist extends LitElement {
  ...
  @property({ type: Boolean, attribute: 'switch-tabpanel' })
  switchTabpanel: boolean = false
  ...
  private setActiveTab(tab: number): void {
    if (this.switchTabpanel) {
      this.parentElement
        ?.querySelector(
          `r-tabpanel[id-prefix="${this.idPrefix}"][index="${this.activeTab}"]`,
        )
        ?.removeAttribute('active')
      this.parentElement
        ?.querySelector(
          `r-tabpanel[id-prefix="${this.idPrefix}"][index="${tab}"]`,
        )
    ...
    this.tablistRef.value
      ?.querySelector(`#${this.idPrefix}-tab-${tab}`)
      ?.focus()
  }
